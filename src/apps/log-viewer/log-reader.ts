...
export class LogReader {
  ...
  public async readLogPage(query: TLogSearchQuery): Promise<TLogReaderResult<TLogPage>> {
  ...
  private buildWhere(query: TLogSearchQuery, includeUpperBound: boolean): TWhereClause {
  ...
  private getRangeStart(range: TLogSearchQuery["range"], until: number): TOption<number> {
    switch (range) {
      case "15m": {
